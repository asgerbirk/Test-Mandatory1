...
describe('FakeInfo Class - Integration Tests', () => {
    it('should generate a complete and consistent fake person', () => {
      ...
      const isValidPrefix = FakeInfo['PHONE_PREFIXES'].some(prefix => phone.startsWith(prefix));
      expect(isValidPrefix).toBe(true);
  
      if (!isValidPrefix) {
        console.error(`Generated phone number ${phone} does not start with a valid prefix`);
      }
    });
    ...
    it('should generate 100 complete and consistent fake person with valid CPR', () => {
        ...
        for (let i = 0; i < 100; i++) {
          ...
          if (isFemale) {
            expect(lastDigit % 2, `Female CPR ${cpr} should end with an even digit (gender: ${gender})`).toBe(0);
          } else {
            expect(lastDigit % 2, `Male CPR ${cpr} should end with an odd digit (gender: ${gender})`).toBe(1);
          }
          ...
          const isValidPrefix = FakeInfo['PHONE_PREFIXES'].some(prefix => phone.startsWith(prefix));
          expect(isValidPrefix, `Phone number ${phone} does not start with a valid prefix`).toBe(true);
        }
      });
